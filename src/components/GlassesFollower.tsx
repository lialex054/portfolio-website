...
export default function GlassesFollower() {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Skew the glasses based on horizontal mouse movement
      const skewX = (e.clientX - window.innerWidth / 2) / window.innerWidth * 20;

      // You can adjust the '10' for more/less skew
      const skewY = (e.clientY - containerCenterY) / window.innerHeight * 20; 

      // Determine if the mouse is on the right half of the page to apply the flip
      ...
      // Combine all transformations: scale for flipping and skew for tilting
      container.style.transform = `scaleX(${scaleX}) skewX(${skewX}deg) skewY(${skewY}deg)`;
    };
  ...
  return (
    ...
    >
      <svg width="171" height="94" viewBox="0 0 342 187" fill="none" xmlns="http://www.w3.org/2000/svg">
